# LifeGrid - Social Media Marketing Kit

## Launch Announcement Posts

### Twitter/X

**Post 1 - Launch Day**
```
Just launched LifeGrid 🟩

Stop watching your life pass. Start building it.

✓ Choose your life path (creator, entrepreneur, engineer...)
✓ Track daily habits in 30 seconds
✓ See your year as a GitHub-style grid

Free on the App Store.

#habittracker #productivity #iOS
```

**Post 2 - Feature Focus**
```
Your life, visualized.

Every green square = a day you showed up for yourself.

LifeGrid turns your daily habits into a contribution grid. Watch your progress grow over time.

Download free: [link]

#selfimprovement #habits #lifegrid
```

**Post 3 - Path Feature**
```
What's your path?

🎬 Content Creator
💡 Entrepreneur
💻 Software Engineer
💪 Fitness
📈 Investor
🧘 Health & Wellness

LifeGrid gives you curated daily habits for YOUR goals.

Choose your path. Build your life.
```

---

### Instagram

**Post 1 - Carousel (use screenshots)**
```
Slide 1: "Your life. One square at a time."
Slide 2: Choose Your Path screenshot
Slide 3: Daily Check-in screenshot
Slide 4: Year Visualized screenshot
Slide 5: "Free on the App Store"

Caption:
Stop watching your life pass by. Start building it. 🟩

LifeGrid is the habit tracker for people who actually want to change.

→ Choose your life path
→ Get curated daily habits
→ Track in 30 seconds
→ Watch your grid turn green

Link in bio.

#habittracker #productivity #selfimprovement #goals #lifegrid #iosapp #dailyhabits
```

**Post 2 - Single Image (use grid screenshot)**
```
156 check-ins. 23 day streak. One green square at a time. 🟩

What would YOUR grid look like?

LifeGrid - Free on the App Store

#consistency #habits #progress #motivation
```

---

### LinkedIn

**Post 1 - Professional Angle**
```
I built an app to solve my own problem.

I wanted to be more consistent with my daily habits. But every app felt like a chore—complex dashboards, too many features, guilt-tripping notifications.

So I built LifeGrid.

The idea is simple:
• Choose your life path (entrepreneur, engineer, creator, etc.)
• Get curated habits that actually matter for your goals
• Check in daily in 30 seconds
• See your effort as a visual grid—like GitHub contributions

Every green square is a day you showed up for yourself.

It's free on the App Store. Would love feedback from this community.

#productivity #habits #ios #entrepreneurship #buildinpublic
```

---

### Reddit

**r/productivity**
```
Title: I built a habit tracker that shows your year as a GitHub contribution grid

Hey r/productivity,

Longtime lurker, first time poster about my own project.

I've tried every habit tracker out there—Streaks, Habitica, Notion templates, you name it. None stuck.

What I really wanted was:
1. Something visual that showed my entire year at a glance
2. A simple daily check-in (not 10 minutes of logging)
3. Habits that actually matched my goals

So I built LifeGrid.

Key features:
- Choose a "life path" (entrepreneur, software engineer, content creator, etc.) and get curated habits
- Daily check-in takes 30 seconds—swipe cards for each habit
- Your progress shows as a GitHub-style grid
- Free tier is actually useful (5 habits, full tracking)

Privacy-focused: all data stays on your device.

Just launched on the App Store. Would love honest feedback.

[link]
```

**r/getdisciplined**
```
Title: Visualizing my habits as a grid changed everything for me

There's something powerful about seeing your entire year as colored squares.

Empty squares = days I didn't show up
Green squares = days I did

No complex analytics. No guilt trips. Just a visual record of consistency.

I built an app around this concept called LifeGrid. It's basically GitHub contributions for your life.

The best part: you choose your "path" (what you're trying to become—entrepreneur, creator, engineer, etc.) and it suggests habits that actually matter for that goal.

Free on the App Store if anyone wants to try it.
```

---

## Hashtag Strategy

**Primary hashtags:**
#lifegrid #habittracker #productivity #selfimprovement #dailyhabits

**Secondary hashtags:**
#iosapp #appstore #consistency #goals #motivation #discipline #buildyourlife #streaks #progressnotperfection

**Niche hashtags:**
#entrepreneurlife #contentcreator #softwareengineer #fitnessjourney #investorlife

---

## Content Calendar (Week 1)

| Day | Platform | Content |
|-----|----------|---------|
| Mon | Twitter | Launch announcement |
| Mon | Instagram | Carousel post |
| Tue | LinkedIn | Professional story post |
| Tue | Reddit | r/productivity post |
| Wed | Twitter | Path feature focus |
| Thu | Instagram | Grid visualization post |
| Thu | Reddit | r/getdisciplined post |
| Fri | Twitter | User benefit focus |
| Sat | Instagram | Story - behind the scenes |
| Sun | Twitter | Week 1 recap / thank you |

---

## Influencer Outreach Template

**Subject:** LifeGrid - Habit tracker you might actually like

**Body:**
```
Hey [Name],

I'm a fan of your content on [productivity/self-improvement/tech].

I just launched LifeGrid—a habit tracker that shows your year as a GitHub-style contribution grid.

What makes it different:
- Choose a "life path" and get curated habits for YOUR goals
- Daily check-in takes 30 seconds
- All data stays on your device (privacy-first)
- Free tier is actually useful

Would love to send you a promo code for Premium if you'd be interested in checking it out. No pressure to post—just think you might genuinely like it.

Thanks for your time,
[Your name]
```

---

## App Store Optimization (ASO) Keywords

**High priority:**
- habit tracker
- daily habits
- goal tracker
- streak tracker
- productivity app

**Medium priority:**
- life planner
- self improvement
- routine tracker
- habit builder
- daily routine

**Long tail:**
- github style habit tracker
- visual habit tracker
- habit tracker with widgets
- minimalist habit tracker
